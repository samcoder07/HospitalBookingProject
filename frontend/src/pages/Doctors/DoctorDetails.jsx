...
const DoctorDetails = () => {
	...
	const { id } = useParams()
	const { data: item, loading, error } = useFetchData(`${BASE_URL}/doctors/${id}`)
	const { name, qualifications, experiences, timeSlots, reviews, bio, about, avgRating, totalRating, specialization, ticketPrice, photo } = item
	return (
		<section>
			<div className='max-w-[1170px] px-5 mx-auto'>
				...
				{!loading && !error && (<div className="grid md:grid-cols-3 gap-[50px]">
						...
						<div className="flex items-center gap-5">
							...
							leading-4 lg:text-[16px] lg:leading-7 font-semibold rounded'>
								...
								<div className="flex items-center gap-[6px]">
									...
									<span className='text-[14px] leading-5 lg:text-[16px] lg:leadig-7 font-[400] text-textColor'>({totalRating})</span>
								</div>
					...
					<div>
						<SidePanel doctorId={item._id} ticketPrice={ticketPrice} timeSlots={timeSlots} />
					</div>
				</div>)}
			</div>
		</section>
